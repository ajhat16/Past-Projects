(* Some simple `assert` style tests that you may modify and extend to
   test functions in the `BSTreeS` signature but also helper functions
   exposed using the `BSTreeImplS` signature.

   You can compile and run this file as follows:
     ocamlbuild BSTree_Impl_Asserts.byte
     ./BSTree_Impl_Asserts.byte
