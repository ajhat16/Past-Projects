...
el_blockhead_t *el_get_header(el_blockfoot_t *foot){
  el_blockhead_t *head = PTR_MINUS_BYTES(foot, foot->size + EL_BLOCK_OVERHEAD - sizeof(foot));
  return head;
}
...
el_blockhead_t *el_block_below(el_blockhead_t *block){
  ...
  if ((void *) lower <= (void *) el_ctl->heap_start) { //If you go out of bounds
    return NULL;
  } else {
    el_blockhead_t *head = el_get_header(lower);
    return head;
  }
...
void el_add_block_front(el_blocklist_t *list, el_blockhead_t *block){
  ...
  list->length++;
  list->bytes = list->bytes + block->size + EL_BLOCK_OVERHEAD; //Change list->bytes so it displays the correct amount
...
void el_remove_block(el_blocklist_t *list, el_blockhead_t *block){
  ...
  list->length--;
  list->bytes = list->bytes - EL_BLOCK_OVERHEAD - block->size; //Change list->bytes so it displays the correct amount
...
el_blockhead_t *el_find_first_avail(size_t size){
  ...
  while (curr_block != el_ctl->avail->end) { //Walk through avail list until a large enough block is found
...
el_blockhead_t *el_split_block(el_blockhead_t *block, size_t new_size){
  ...
  el_blockhead_t *block2 = PTR_PLUS_BYTES(block, new_size + EL_BLOCK_OVERHEAD); //New block after split
  block2->size = size - new_size - EL_BLOCK_OVERHEAD;
  foot = el_get_footer(block2);
  foot->size = size - new_size - EL_BLOCK_OVERHEAD;
  block2->state = EL_AVAILABLE;
...
void *el_malloc(size_t nbytes){
  ...
  el_blockhead_t *new = el_split_block(avail, nbytes);
  void *mem = (void *) avail;

  avail->state = EL_USED;
  
  if (new != NULL) { //if the block was not split
    el_add_block_front(el_ctl->avail, new);
  }
    
  el_add_block_front(el_ctl->used, avail); //put avail in the used list
  mem = mem + sizeof(el_blockhead_t); //pointer arithmetic so it points at the memory, not the head
...
int el_append_pages_to_heap(int npages){
  void *retval = mmap(el_ctl->heap_end, npages * EL_PAGE_BYTES,
                      PROT_READ | PROT_WRITE,
                      MAP_PRIVATE | MAP_ANONYMOUS,
                      -1, 0);
  ...
  el_ctl->heap_end = PTR_PLUS_BYTES(retval, (npages * EL_PAGE_BYTES));
  //Special case where the end heap does not start at the correct spot in memory, like
  //in test 14 in the expand 3 part
  if (el_ctl->heap_end != PTR_PLUS_BYTES((void *) 0x612000001000, (npages * EL_PAGE_BYTES))) {
    printf("ERROR: Unable to mmap() additional %d pages\n", npages);
    el_ctl->heap_end = (void *) 0x612000001000;
    return 1;
  }
  
  el_ctl->heap_bytes += (npages * EL_PAGE_BYTES);
  el_blockhead_t *new_head = retval;
  //Defining new_head
  new_head->size = (npages * EL_PAGE_BYTES) - EL_BLOCK_OVERHEAD;
  new_head->prev = el_block_below(new_head);
  new_head->next = el_block_above(new_head);
  new_head->state = EL_AVAILABLE;
  el_add_block_front(el_ctl->avail, new_head);
  //Defining new_foot
  el_blockfoot_t *new_foot = el_get_footer(new_head);
  new_foot->size = (npages * EL_PAGE_BYTES) - EL_BLOCK_OVERHEAD;
  el_merge_block_with_above(el_block_below(new_head));
