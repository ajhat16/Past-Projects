        ...
        api.onmessage = function (msg, data) {
            if ("event" in data) {
                ...
                if (data.event == "UpdateEntity") {
                    var e = data.details;
                    // console.log(`update entity: ${e} - ${e.xp}`);
                    entityXP[e.id] = e.xp;

                    document.getElementById("lblXP").innerText = `${Math.round((entityXP[currentView]+ Number.EPSILON)*100)/100}`;
...
const onLoad = (gltf, position, scale, start, duration, details, id) => {
    ...
    var sphereMaterial = new THREE.MeshBasicMaterial({
        color: parseInt(details.color, 16),
        transparent: true,
        opacity: opacity
    });
...
function removeEntity(id) {
    ...
    models = models.filter(function (value, index, arr) {
        return value != model;
    });
    scene.remove(model);
...
const UPGR_ID_SPEED = 1;
const UPGR_ID_COLOR = 2;

let currentUpgrade;
let currentColor;

const ID_UPGR_SPEED = '#upgrSpeed';

/**
 * @author Read S
 */
function addUpgrade() {
    currentUpgrade = $("#upgradeSelect").val();
    const currentEntity = entityList[currentView];
    const currentSpeed = $(ID_UPGR_SPEED).val();
    currentColor = $("#colorSelect").val();
    let upgrVal = null;

    if (currentUpgrade == UPGR_ID_SPEED) {
        upgrVal = currentSpeed;
    }
    if (currentUpgrade == UPGR_ID_COLOR) {
        upgrVal = currentColor;
    }

    console.log(`${currentUpgrade} for ${currentEntity} ${upgrVal}`);

    api.sendCommand("AddUpgrade", {
        "type": Number(currentUpgrade),
        "entity": Number(currentEntity),
        "value": Number(upgrVal),
    });
...
function resetUpgrade() {
    ...
    api.sendCommand("ResetUpgrade", {
        "type": Number(currentUpgrade),
        "entity": Number(currentEntity),
    });

    console.log(`${currentUpgrade} for ${currentEntity} reset`);

}

function changeSpeed() {
    const currentSpeed = $(ID_UPGR_SPEED).val();
    document.getElementById("lblSpeed").innerText = `${currentSpeed}`;
}
...
function selectUpgrade() {
    currentUpgrade = $("#upgradeSelect").val();
    console.log(`upgrade selected: ${currentUpgrade} - ${entityXP[currentView]}`);

    if (currentUpgrade == UPGR_ID_COLOR) {
        document.getElementById("sm-colors").hidden = false;
        document.getElementById("sm-speed").hidden = true;
    } else if (currentUpgrade == UPGR_ID_SPEED) {
        document.getElementById("sm-colors").hidden = true;
        document.getElementById("sm-speed").hidden = false;
    } else {
        document.getElementById("sm-colors").hidden = true;
        document.getElementById("sm-speed").hidden = true;
    }
