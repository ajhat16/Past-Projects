...
#include "../../../libs/transit/include/Upgrade.h"
#include "../../../libs/transit/include/IUpgradeable.h"
...
public:
	...
	void ReceiveCommand(const std::string& cmd, JsonObject& data, JsonObject& returnValue)
	{
		...
		const std::string CMD_UPGRADE  = "AddUpgrade";
		const std::string CMD_UPGRADE2 = "ResetUpgrade";
...
//			printf("update received\n");
			...
			std::cout << "Received upgrade command\n";

			Upgrade* u   = new Upgrade(data);
			IEntity* ent = Find(u->GetEId());
			Vector3 vec = ent->GetPosition();
			u->ApplyUpgrade(ent);
			if (u->GetId() == UpgradeId::COLOR) {
				SendEntity("ColorUpgrade", *ent, true);
			}
			delete u;
			printf("found entity %d\n", ent->GetId());
			vec.Print();
			std::cout << std::endl;
			UpdateEntity(*ent);

		}
		else if (cmd == CMD_UPGRADE2) {
			Upgrade* u   = new Upgrade(data);
			IEntity* ent = Find(u->GetEId());
			u->ResetUpgrade(ent);
			UpdateEntity(*ent);
	...
	/**
	 * @brief Finds an entity by its index
	 * @param i Entity index
	 * @return Pointer to entity; nullptr if not found
	 * @author Read S
	 */
	inline IEntity* Find(int i)
	{
		for (std::map<int, const IEntity*>::iterator it = updateEntites.begin(); it != updateEntites.end(); it++) {
			auto id = it->first;
			if (id == i) {
				IEntity* ent = const_cast<IEntity*>(it->second);
				return ent;
			}
		}
		return nullptr;
	...
	void SendEntity(const std::string& event, const IEntity& entity, bool includeDetails)
	{
        ...
        if(col_ != COLOR_DEFAULT) details["color"] = col_;
		float xp = dynamic_cast<IUpgradeable&>(const_cast<IEntity&>(entity)).GetXP();
//		printf("xp: %f for %d\n",xp,entity.GetId());
		details["xp"]= xp;
...
protected:
	...
	{ return new TransitService(model); }
