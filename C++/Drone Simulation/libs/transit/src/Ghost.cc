...
Ghost::Ghost(JsonObject& obj) : details(obj) {
...
void Ghost::CalcAndSetDestination() {
  int rand_x, rand_z;
  srand(time(0));
  rand_x = rand() % 1500;
  ...
  SetDestination(Vector3(rand_x, 260, rand_z));
  destination_set = true;
  
}

void Ghost::Update(double dt, std::vector<IEntity*> scheduler) {
  if (!destination_set) {
    CalcAndSetDestination();
  }
