...
class DijkstraStrategy : public IStrategy {
 public:
  /**
  * @brief Constructor for the DijkstraStrategy class; initializes the value for
  ...
  * to set the starting position of the path that will be generated using graph.
  ...
  * graph.
  * @param graph Used along with position and destination to create the movement
  * path for the entity to follow.
  **/
  DijkstraStrategy(Vector3 position, Vector3 destination, const IGraph* graph);

  /**
  * @brief Destructor for the DijkstraStrategy class; automatically deletes any
  * allocated variables.
  **/
  ~DijkstraStrategy();

  /**
  * @brief This function moves the entity along the path set in the constructor
  * of DijkstraStrategy. The path is made up of nodes, so after a node is
  * reached, the function automatically switches to the next node until the
  * entity reaches its destination (the last node).
  * @param entity This is the entity that will be moving using the Dijkstra
  * Strategy.
  ...
  * destination by comparing two private member variables used to track where in
  * the path the entity is.
